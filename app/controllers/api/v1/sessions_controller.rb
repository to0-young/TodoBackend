class Api::V1::SessionsController < ApplicationController
  ...
  def create
    if !cookies[:session].blank?
      return render json: { message: 'Session already active' }, status: unauthorized
    end
    ...
    if user.nil?
      ...
      return render json: { message: "There is no such user" }, status: :not_found
    else
      ...
      if correct_password == false
        ...
        return render json: { message: 'Password not valid' }, status: :unauthorized
      end
    end
